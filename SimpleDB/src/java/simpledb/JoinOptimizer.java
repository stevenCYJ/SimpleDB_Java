...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // If hash joind, use additions
            if (j.p == Predicate.Op.EQUALS) {
                return (double) card1 + (double) card2 + cost1 + (cost2 * Math.max((double) card1 / (double) HashEquiJoin.MAP_SIZE, 1));
            } else
                // If hash is nested, use multiplication
...
//                                        joinOp) * card1);
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        Vector<LogicalJoinNode> best_Sol = new Vector<LogicalJoinNode>();
        int minCost_Card = 0;
        double minCost_Subset = 0.0;

        PlanCache pc = new PlanCache();

        for (int i = 0; i < joins.size(); i++) {
            //Traverse the joint nodes 
            Set<Set<LogicalJoinNode>> subsets = enumerateSubsets(joins, i + 1);
            for (Set<LogicalJoinNode> s : subsets) {
                minCost_Subset = Double.MAX_VALUE;
                minCost_Card = 0;
                best_Sol = new Vector<LogicalJoinNode>();
                
                //for each node, return an optimal way to add and remove itself
                for (LogicalJoinNode j : s) {
                    CostCard cc;
                    //compute the total cost for subplan
                    cc = computeCostAndCardOfSubplan(stats,
                            filterSelectivities, j, s, minCost_Subset, pc);
                    if (cc == null)
                        continue;
                    //update each temp values
                    best_Sol = cc.plan;
                    minCost_Subset = cc.cost;
                    minCost_Card = cc.card;
                }
                //add the best value to the current pc
                if (minCost_Subset != Double.MAX_VALUE) {
                    pc.addPlan(s, minCost_Subset, minCost_Card, best_Sol);
                }

            }
        }
        if (explain)
            printJoins(best_Sol, pc, stats, filterSelectivities);
        return best_Sol;
    }
